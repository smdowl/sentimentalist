...
Having run this, we can perform another map the splits by time segment such that the reducer can then create a full feature vector for each time slice as we see fit.
...
This component will be the central component of change in order to explore different options in this phase.
The Extractor will also have access to another variable global stats file tuned to the needs of the specific Extractor implementation. 
The statistics available and the manor with which they are gathered will again be an area of experimentation.
...
\begin{itemize}
\item entity mentions
\item sentiment
\item word frequencies
\item user ID
\item number of re-tweets
\end{itemize}

\Subhead{Global Statistics}

\sf
We wil use the other MapReduce job to gather global user or corpus statistics such as:
\begin{itemize}
\item user average sentiment
\item inverse document frequency
\item cliques of users (strongly connected groups)
\end{itemize}
